...
# input path
inpath = base + 'emoji.h5'
# output path
outbase = base + '/emoji_result/{}/'.format(latent_dim)
# saved model and weights
mpath = outbase + 'emoji_model_dim={}.json'.format(latent_dim)
wpath = outbase + 'emoji_model_dim={}.h5'.format(latent_dim)
logpath = outbase + 'emoji_log_dim={}.csv'.format(latent_dim)
...
def train ():
    ...
    m = model.Vae(latent_dim = latent_dim, img_dim=(4, 64, 64))
...
def visualize (x_test, encoder, generator, suffix=''):
    ...
    img = Image.fromarray(reconstructed, 'RGBA')
